...
| :exclamation: Описание |
|-|
|При первом запуске создаётся пустое поле с расходами, пустое поле с категориями, пустое поле с бюджетом.<br />Создание/удаление категорий и расходов происходит через вызов контектсного меню в соответвующем виджете **ПКМ**.<br />При попытке создать расход при невыбранной или некоректной категории будет отображено окно с ошибкой.<br />При попытке создать или изменить категорию с уже имеющемся именем будет отобраджено окно ошибки и выставится имя _err (Wont be uploaded). Данная категория считается некорректной.<br />При удалении категории **удаляются** все дочерние. При этом во всех расходах, использующих удалённые категории будут выставленные первые доступные категории, либо некорректные. В случае выставления некорректных категоий, если они не будут исправлены, при следующем запуске данные записи **удалятся**.<br />Изменение бюджета возможно через соовтетствующие ячейти таблице. Ячейки расходов обновляются автоматически за соответствующий период, начиная с даты обновления.<br />Ячейки проверяются на валидность введённых данных, но не защищены от SQL инъекций.<br />Редактирование проиходит с помощью **двойного щелчка**.<br />Запуск: `poetry run python bookkeeper/ui_client.py`|
